...
def sync_database(config=None):
    ...
    movies_data_to_insert = []
    hardlinks_folders_to_delete = []
    hardlinks_data_to_insert = []
    ...
    for movie in all_movies:
        ...
        for hl in all_hardlinks:
            hardlinks_data_to_insert.append((
                folder_name, hl.get('genre'), hl.get('folder'), hl.get('found', 0), hl.get('total', 0), 1 if hl.get('exists') else 0, hl.get('type')
            ))

    # ⚡ Bolt Optimization: Use executemany for batch inserts/deletes to avoid N+1 query parsing and disk fsync overhead
    if movies_data_to_insert:
        cursor.executemany('''
