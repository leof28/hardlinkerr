...
def sync_database(config=None):
    ...
    # ⚡ Bolt Optimization: Batch database operations with executemany to reduce fsync and query parsing overhead
    movies_data = []
    hardlinks_delete_data = []
    hardlinks_insert_data = []
    ...
    for movie in all_movies:
        ...
        hardlinks_delete_data.append((folder_name,))
        for hl in all_hardlinks:
            hardlinks_insert_data.append((folder_name, hl.get('genre'), hl.get('folder'), hl.get('found', 0), hl.get('total', 0), 1 if hl.get('exists') else 0, hl.get('type')))
    ...
    if hardlinks_delete_data:
        cursor.executemany('DELETE FROM hardlinks WHERE movie_folder = ?', hardlinks_delete_data)

    if hardlinks_insert_data:
        cursor.executemany('''
