...
def sync_database(config=None):
    ...
    if to_delete:
        # ⚡ Bolt Optimization: Batch SQLite DELETE operations to avoid O(N) query parsing and disk fsync overhead
        delete_params = [(fd,) for fd in to_delete]
        cursor.executemany('DELETE FROM movies WHERE folder_name = ?', delete_params)
        cursor.executemany('DELETE FROM hardlinks WHERE movie_folder = ?', delete_params)
