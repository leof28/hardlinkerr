...
def is_valid_folder_name(name):
    """Rejects directory separators and parent shortcuts to prevent lateral traversal."""
    if not name:
        return True
...
def delete_movie():
    ...
    if not is_valid_folder_name(folder_name):
        append_log("warning", "security", f"Tentative de traversée latérale détectée: {folder_name}")
...
def delete_hardlink_folder():
    ...
    if not is_valid_folder_name(folder_name) or not is_valid_folder_name(target_folder):
        append_log("warning", "security", f"Tentative de traversée latérale détectée: {folder_name} dans {target_folder}")
        return jsonify({"error": "Nom de dossier invalide"}), 400
