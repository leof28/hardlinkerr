...
def sync_database(config=None):
    ...
    if to_delete:
        to_delete_tuples = [(fd,) for fd in to_delete]
        cursor.executemany('DELETE FROM movies WHERE folder_name = ?', to_delete_tuples)
        cursor.executemany('DELETE FROM hardlinks WHERE movie_folder = ?', to_delete_tuples)
