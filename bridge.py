...
def sync_database(config=None):
    ...
    for movie in all_movies:
        ...
        for hl in all_hardlinks:
            hardlinks_to_insert.append(
                (folder_name, hl.get('genre'), hl.get('folder'), hl.get('found', 0), hl.get('total', 0), 1 if hl.get('exists') else 0, hl.get('type'))
            )

    # ⚡ Bolt Optimization: Replace O(N) database operations inside loop with batched executemany()
    # Impact: Reduces database query parsing overhead and disk fsyncs from O(N) to O(1)
    if movies_to_upsert:
        cursor.executemany('''
    ...
    to_delete = [(fd,) for fd in (db_folders - current_folders)]
    if to_delete:
        cursor.executemany('DELETE FROM movies WHERE folder_name = ?', to_delete)
        cursor.executemany('DELETE FROM hardlinks WHERE movie_folder = ?', to_delete)
