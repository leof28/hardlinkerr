...
def is_valid_folder_name(name):
    """Vérifie que le nom de dossier ne contient pas de séparateurs ou de raccourcis parent pour éviter le 'lateral traversal'."""
    if not name:
        return True
...
def delete_file():
    ...
    try:
        if file_path and os.path.exists(file_path):
            ...
            # Explicitly validate the derived parent directory before deletion to prevent unintended removal of application root folders (e.g., attacker passing <root>/dummy.txt)
...
def delete_movie():
    ...
    if not is_valid_folder_name(folder_name):
        append_log("warning", "security", f"Tentative de traversée latérale détectée dans folderName: {folder_name}")
...
def delete_hardlink_folder():
    ...
    if not is_valid_folder_name(folder_name) or not is_valid_folder_name(target_folder):
        append_log("warning", "security", f"Tentative de traversée latérale détectée: {folder_name} ou {target_folder}")
        return jsonify({"error": "Nom de dossier invalide"}), 400
