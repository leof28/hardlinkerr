...
def sync_database(config=None):
    ...
    for movie in all_movies:
        ...
        movies_folders.append((folder_name,))
    ...
    cursor.executemany('DELETE FROM hardlinks WHERE movie_folder = ?', movies_folders)
    cursor.executemany('''
    ...
    to_delete_folders = [(fd,) for fd in to_delete]
    cursor.executemany('DELETE FROM movies WHERE folder_name = ?', to_delete_folders)
    cursor.executemany('DELETE FROM hardlinks WHERE movie_folder = ?', to_delete_folders)
