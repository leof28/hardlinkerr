...
def delete_movie():
    ...
    if folder_name and ('/' in folder_name or '\\' in folder_name or folder_name == '..'):
        append_log("warning", "security", f"Tentative de path traversal (folder_name): {folder_name}")
...
def delete_hardlink_folder():
    ...
    if '/' in folder_name or '\\' in folder_name or folder_name == '..':
        return jsonify({"error": "Nom de dossier invalide"}), 400
    if '/' in target_folder or '\\' in target_folder or target_folder == '..':
