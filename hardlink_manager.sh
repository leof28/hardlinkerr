...
# --- CHARGEMENT DU MAPPING DES STUDIOS ---
declare -A STUDIO_TO_LOCAL
IFS='|' read -ra STUDIO_ADDR <<< "$STUDIO_MAPPING_STR"
for i in "${STUDIO_ADDR[@]}"; do
    IFS=':' read -r r_studio l_studio <<< "$i"
    if [ -n "$r_studio" ]; then
        STUDIO_TO_LOCAL["$r_studio"]="$l_studio"
    fi
done
...
echo "$movies_json" | jq -r '.[] | select(.hasFile == true) | [.title, .path, (.genres | map(if type == "object" then .name else . end) | join(",")), (.studio // "")] | @tsv' | while IFS=$'\t' read -r title folder_path movie_genres_raw movie_studio_raw; do
    folder_name="${folder_path##*/}"
    movie_studio="${movie_studio_raw//$'\r'/}"
    IFS=',' read -ra movie_genres <<< "$movie_genres_raw"
    ...
    if [ -n "$movie_studio" ]; then
        if [[ -z "$SPECIFIC_STUDIOS" || "$movie_studio" == "$SPECIFIC_STUDIOS" ]]; then
            studio_local_folder="${STUDIO_TO_LOCAL[$movie_studio]}"
            if [ -n "$studio_local_folder" ]; then
                target_folders["$studio_local_folder"]="studio:$movie_studio"
            fi
        fi
    fi
    ...
    for local_folder in "${!target_folders[@]}"; do
        category_info="${target_folders[$local_folder]}"
        link_type="${category_info%%:*}"
        ...
        if [ "$status_only" -eq 1 ]; then
            ...
            if [ ! -d "$target_dir" ]; then
                ...
                echo "HARDLINK_STATUS|$folder_name|$category_name|$local_folder|0|$total_files|$link_type"
                continue
            fi
            ...
            for src_file in "${all_files[@]}"; do
                ...
                # ⚡ Bolt Optimization: Replace subshell $(basename "$src_file") with native string parameter expansion
                dest_file="$target_dir/${src_file##*/}"

                # ⚡ Bolt Optimization: Use bash builtin -ef to check if files share same device/inode, avoiding O(N) subshells for stat
            ...
            # Afficher le statut
            echo "HARDLINK_STATUS|$folder_name|$category_name|$local_folder|$found_valid|$total_files|$link_type"
        else
            ...
            for src_file in "${all_files[@]}"; do
                ...
                # ⚡ Bolt Optimization: Use bash builtin -ef instead of subshell stat calls
