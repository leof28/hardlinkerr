...
## 2026-08-30 - [Accessible Tabbed Navigation]
**Learning:** When building custom tabbed navigation components in React, using `div` or `button` elements without ARIA roles prevents screen readers from understanding the structure. Explicitly adding `role="tablist"` to the container, `role="tab"`, `aria-selected`, and `aria-controls` to the tabs, and `role="tabpanel"` to the content area ensures proper semantic interpretation.
**Action:** Always implement the WAI-ARIA tablist pattern (`tablist`, `tab`, `tabpanel`, `aria-selected`, `aria-controls`) when replacing native browser navigation with custom interactive tabs in a single-page application.
