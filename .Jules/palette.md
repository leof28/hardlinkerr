...
## 2026-09-02 - [Tab Navigation Accessibility]
**Learning:** When implementing custom tabbed navigation interfaces in standalone React setups, applying `role="tablist"` to the tab container, `role="tab"` to the buttons with `aria-selected` and `aria-controls`, and a corresponding `role="tabpanel"` area is necessary for screen reader users to understand the component structure and navigation state.
**Action:** Always wrap tab content in a `tabpanel` and structure tab buttons with `tablist`/`tab` roles.
