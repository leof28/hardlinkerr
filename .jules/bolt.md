...
## 2026-07-28 - SQLite batch insert/update optimization
**Learning:** Repeated execution of single `cursor.execute()` for updates or deletes inside loops causes significant query parsing and disk fsync overhead.
**Action:** Use `cursor.executemany()` to execute changes in batches to avoid this N+1 query parsing and disk fsync bottleneck.
